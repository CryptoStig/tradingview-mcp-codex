...
export function registerMorningTools(server) {
  server.tool(
    "morning_brief",
    "Scan your watchlist, read all indicator values, and return structured data for a session brief. Reads rules.json for your bias criteria and watchlist. Your assistant applies the rules to generate your daily bias.",
    {
      rules_path: z
        .string()
        .optional()
  ...
  server.tool(
    ...
    "Save today's morning brief to ~/.tradingview-mcp/sessions/YYYY-MM-DD.json for future reference.",
    {
      brief: z
        .string()
        .describe(
          "The brief text to save (output from morning_brief after your assistant applies the rules).",
        ),
      date: z
        .string()
        .optional()
